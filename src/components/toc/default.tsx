...
function ThumbTrack({ computed, thumbBox }: { computed: ComputedSVG; thumbBox: boolean }) {
  ...
  const trackStyle = useMemo(() => {
    ...
    const isActive = (item: Primitive.TOCItemType) =>
      activeAnchors.includes(item.url.slice(1));
    const startIdx = items.findIndex(isActive);
    if (startIdx === -1) return out;

    const endIdx = items.findLastIndex(isActive);
    ...
    return out;
  }, [items, activeAnchors, computed, thumbBox]);
